...
//    }
    ...
    memcpy (&bag_image.data[0], sf->small_img, (SMALL_IMG_HEIGHT * SMALL_IMG_WIDTH) * sizeof (uint8_t));
        ...
        {
            fprintf(fp2, "P5\n640 480\n255\n");
            fwrite(&bag_image.data[0], 640, 480, fp2);
            fclose(fp2);
        }

    bag_image.height = SMALL_IMG_HEIGHT;
    bag_image.width = SMALL_IMG_WIDTH;
    bag_image.encoding = "mono8";
    bag.write ("tango/small_image", bag_image.header.stamp, bag_image);

    bag_image.header.seq += 1;
    bag.write ("tango/small_image", bag_image.header.stamp + ros::Duration(1), bag_image);
    fclose(fp);
